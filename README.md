...
로보77 두 플레이어가 번갈아가며 카드를 내고, 카드의 숫자 합계가 **77을 초과하지 않도록** 하는 게임이다.

플레이어는 숫자 카드와 특수 카드를 전략적으로 사용하여 상대방이 패배하도록 유도한다.

이번 프로젝트는 이를 **객체지향적으로 설계한 콘솔 버전**과 **JDA 기반 디스코드 봇 버전**으로 구현하여,

1–3주차 동안 쌓아온 나만의 객체지향 기준을 더 복잡하고 현실적인 상황에서 직접 적용해보는 도전을 해보고자 한다.

예측하기 어려운 흐름 속에서 기준이 어떤 순간에 흔들리고 또 어디에서 빛나는지를 체감하며 더욱 단단하고 유연한 기준으로 발전시키는 것이 목표다.
