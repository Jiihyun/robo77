...
class DoubleTurnPolicyTest {
    ...
    void 다음_순서의_플레이어를_구한다() {
        ...
        DoubleTurnPolicy doubleTurnPolicy = new DoubleTurnPolicy();
        // when
        Player nextTurnPlayer = doubleTurnPolicy.findNextTurnPlayer(players);
        // then
        assertAll(
                () -> assertThat(nextTurnPlayer).isEqualTo(player2),
                () -> assertThat(List.of(players.pollFirst(), players.pollFirst(), players.pollFirst()))
                        .containsExactly(player2, player2, player1)
        );
    }
